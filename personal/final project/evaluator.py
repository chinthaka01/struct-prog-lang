...
def evaluate(ast, environment):
    ...
    if ast["tag"] == "assign":
        ...
        if len(environment) > 0 and target_base[target_index] != None:
            existingValueType = type_of(target_base[target_index])
            newValueType = type_of(value)
            if existingValueType != newValueType:
                raise Exception(f"Invalid {ast['tag']} to identifier '{target_index}': cannot assign a value of type '{newValueType}' to a variable of type '{existingValueType}'.")
