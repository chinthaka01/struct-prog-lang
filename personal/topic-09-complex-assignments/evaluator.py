...
def evaluate(ast, environment, watch=None):
    ...
    if ast["tag"] == "identifier":
        ...
        if "$parent" in environment:
            return evaluate(ast, environment["$parent"], watch)
    ...
    if ast["tag"] == "+":
        ...
        if types == "number-number":
            changeLocation = inspect.currentframe().f_back.f_lineno
    ...
    if ast["tag"] == "assign":
        ...
        watchIndentifier(watch, target["tag"], value);
...
def watchIndentifier(watchingIdentifier, tag, value):
    global changeLocation
    if watchingIdentifier is not None:
        if tag == "identifier":
            print(f"WATCH: {watchingIdentifier} = {value}  (change location {changeLocation})")
    changeLocation = None
